FormModel = Backbone.Model.extend({
  ...
  dumpChunk:function (name,chunk) {
    ...
    $fh.logger.debug("dumpChunk size='" , chunk.fileBase64.length);
  },
  ...
  handleError: function(msg, cb) {
    $fh.logger.debug("handleError", msg);
  ...
  pollRemoteFormSubmissionComplete: function(req,form_id,res, cb) {
    ...
    $fh.logger.debug('Form Submitted to cloud: res:', res);
    ...
    async.whilst(
      ...
      function process(callback) {
        setTimeout(function () {
          poll(function (res) {
            $fh.logger.debug('pollRemoteFormSubmissionComplete process : res=', res);
      ...
      function complete(res) {
        $fh.logger.debug('pollRemoteFormSubmissionComplete complete : res=', res);
        if(res) {
          if (res.Success === 1){
  ...
  submitChunk: function(req,chunk, callback) {
    ...
    var tmp  =  _.omit(value, "fileBase64");
    var len =  value.fileBase64.length

    tmp["fileBase64"]  =  value.fileBase64.length;
    $fh.logger.debug("submitChunk starting value="  , tmp);
    $fh.act({
      "act":"submitChunk",
      "retries" : App.config.get("max_retries"),
      "req": chunk
    }, function (res) {
      $fh.logger.debug("submitChunk starting form[" +chunk.form_id + "][" + chunk.name+ "] res='" , res , "'");
      if (res.Success && res.Success === 1) {
        req.size += len;
      ...
      $fh.logger.debug("submitChunk starting form[" +chunk.form_id + "][" + chunk.name+ "] msg='" , msg ,"' err='" , err , "'");
    ...
    if(App.config.get("use_chunking")) {
      _.each(serialized_form, function chunkHandler(value,name){
        if (_.isObject(value) && !_.isUndefined(value.filename)) {
          ...
          $fh.logger.debug("field name=" + name + ",size=" + size);
