...
exports.getForm = function(params, callback) {
  console.log(new Date() + ' :: exports.getForm');
  getConfig(function(err, wufoo_config) {
    //console.log('after getConfig');
    if (err != null) {
      return callback(null, err);
      ...
      if (cacheable()) {
            ...
            if (typeof wufoo_config.wufoo_config.form_password != 'undefined' && wufoo_config.wufoo_config.form_password) {
              var req = request({
                ...
                updateWufooHTML(form_hash, updated, body, false, function(processed_html) {
                  console.log(new Date() + ' :: updateWufooHTML 1');
                  return callback(null, {
                    "html": processed_html
                  });
                });
              ...
              request(url, function(error, res, body) {
                updateWufooHTML(form_hash, updated, body, false, function(processed_html) {
                  console.log(new Date() + ' :: updateWufooHTML 2');
                  return callback(null, {
                    "html": processed_html
                  });
                });
        ...
        if (typeof wufoo_config.wufoo_config.form_password != 'undefined' && wufoo_config.wufoo_config.form_password) {
          var req = request({
            ...
            updateWufooHTML(form_hash, updated, body, false, function(processed_html) {
              console.log(new Date() + ' :: updateWufooHTML 3');
              return callback(null, {
                "html": processed_html
              });
            });
          ...
          request(url, function(error, res, body) {
            updateWufooHTML(form_hash, updated, body, false, function(processed_html) {
              console.log(new Date() + ' :: updateWufooHTML 4');
              return callback(null, {
                "html": processed_html
              });
            });
...
exports.getForms = function(params, callback) {
  console.log(new Date() + ' :: exports.getForms');
  getConfig(function(err, wufoo_config) {
    if (err != null) {
      return callback(null, err);
    }
    ...
    if (app_type == 'single_form') {
      ...
      request.get({
        ...
        console.log(new Date() + ' :: request.get');
        return callback(null, {
          data: JSON.parse(body)
        });
      });
