...
exports = module.exports = function (opts, cb)
{
  ...
  //console.log('TIMER - t3 - ' + (Date.now() - start));
  async.parallel([function (oCallback)
  {
    ...
    //console.log('TIMER - at1 - ' + (Date.now() - start));
    ...
    if (sigFields.length > 0)
    {
      $.each(sigFields, function (i, field)
      {
        ...
        //console.log("Sigfield html is " + $(field).html());
      });
    }
    //console.log('TIMER - at2 - ' + (Date.now() - start));
    // API binding names
    ...
    function fieldLogic(field, classes)
    {
      ...
      if (type === 'file' && classes.indexOf('fhcam') !== -1)
      {
        ...
        //console.log("Camfield html is " + $(field).html());
      }
    }
    //console.log('TIMER - at3 - ' + (Date.now() - start));
    ...
    //console.log('TIMER - at4 - ' + (Date.now() - start));
    //Process map field
    ...
    //console.log('TIMER - atEND - ' + (Date.now() - start));
    // all jquery manipulation stuff done, async finished
    oCallback(null);
  }, function (oCallback)
  {
    //console.log('TIMER - st1 - ' + (Date.now() - start));
    var inlineCounter = 0;
    var scripts = $('script');
    ...
    }, function (err)
    {
      ...
      //console.log('TIMER - stEND - ' + (Date.now() - start));
      oCallback(null);
    });
  }, function (oCallback)
  {
    //load links parallellellellellelly
    //console.log('TIMER - lt1 - ' + (Date.now() - start));
    ...
    }, function (err)
    {
      ...
      //console.log('TIMER - ltEND - ' + (Date.now() - start));
      oCallback(null);
    });
  }, function (oCallback)
  {
    // load images, base64 encode them, and embed them in html
    //console.log('TIMER - it1 - ' + (Date.now() - start));
    var imgs = $('img');
    ...
    }, function (err)
    {
      ...
      //console.log('TIMER - itEND - ' + (Date.now() - start));
      oCallback(null);
    });
  }], function (err, results)
  {
    ...
    var styles = $('style');
    //console.log('TIMER - ct1 - ' + (Date.now() - start));
    async.forEach(styles, function (item, aCallback)
    {
      ...
      var matches = [];
      while (match = r.exec(styleText))
      { // apply regex and check if defined all in one go
        matches.push(match);
      }
    ...
    }, function (err)
    {
      ...
      //console.log('TIMER - ctEND - ' + (Date.now() - start));
      var subBtnType = $('.submit').attr('type');
      //console.log("submit button type is " + subBtnType);
      ...
      //console.log('TIMER - pt1 - ' + (Date.now() - start));
      ...
      //console.log('TIMER - ptEND - ' + (Date.now() - start));
      //console.log('TIMER - end ' + (Date.now() - start));
      cb(null, processed_html);
    });
  });
};
var getRemoteResource = function (path, cb, isBinary)
  {
    var options = {};
    ...
    }, function (err, res)
    {
      ...
      {
        console.log("found cached data for uri : " + options.uri);
        cb(null,
        {
          statusCode: 200
        }, res);
